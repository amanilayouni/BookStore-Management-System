...
router.post('/add',  checkRole.checkRole, (req, res) => {
    ...
    connection.query(query, [product.name, product.categoryId, product.description, product.price], (err, results) => {
        if (!err) {
            return res.status(200).json({ message: "produit ajoutee avec succés 👍" })
        }
    ...
    connection.query(query, [product.name, product.categoryId, product.description, product.price, product.id], (err, results) => {
        if (!err) {
            if (results.affectedRows == 0) {
                return res.status(404).json({ message: "product id n'existe pas !" });
            }
            return res.status(200).json({ message: "produit modifiee avec succes :)" });
        }
        else {
            return res.status(500).json(err);
        }
...
router.delete('/delete/:id', checkRole.checkRole, (req, res, next) => {
    ...
    connection.query(query, [id], (err, results) => {
        if (!err) {
            ...
            return res.status(200).json({ message: "produit supprimee avec succes :)" });
        }
        else {
            return res.status(500).json(err);
        }
...
router.patch('/updateStatus',  checkRole.checkRole, (req, res, next) => {
    ...
    connection.query(query, [user.status,user.id], (err, results) => {
        if (!err) {
            ...
            return res.status(200).json({ message: "produit status modifiee avec succes :)" });
        }
        else {
            return res.status(500).json(err);
        }
